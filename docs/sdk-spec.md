...
export type Result<T, E = Err> = T | E;
```

规则：

- 默认错误哨兵为 `Err`
- 需要扩展错误语义时，使用受控错误 tag，而不是抛异常
- 核心路径禁止把异常当普通分支机制

边界例外：

- 外部 API、JSON 解析、宿主环境调用若会抛错，必须在边界捕获并立即转译为 `Err` 或受控错误 tag
- Zod 解析失败必须在边界立即转译，不能把 `ZodError` 透传进核心层
- 异常不能穿透进入核心域
...
function parseUserInput(input: unknown): Result<User> {
...
```ts
function parsePort(input: string): Result<SmiInt> {
...
function parsePortFromUnknown(input: unknown): Result<SmiInt> {
